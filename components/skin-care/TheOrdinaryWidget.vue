<template>
  ...
  <nuxt-link :to="link" class="listing-item-container compact">
    <div class="listing-item">
      ...
      <!-- <p>{{ getFirstParagraph(post_skin_care) }}</p> -->
    </div>
  </nuxt-link>
</template>
...
export default {
  ...
  name: "the-ordinary-widget",
  methods: {
    // Function to get the first paragraph of text in a blog post and limit the displayed text at 300 characters
    getFirstParagraph(post_skin_care) {
      const textLimit = 300;
      const slices = post_skin_care.data.body;
      let firstParagraph = "";
  ...
  created() {
    (this.link = LinkResolver(this.post_skin_care)),
      (this.formattedDate = Intl.DateTimeFormat("en-US", {
        year: "numeric",
        month: "short",
        day: "2-digit"
      }).format(new Date(this.post_skin_care.data.date)));
  }
};
</script>
